    ...
    private fun setHandlers() {
        ...
        try {
            binding.addNote.setOnClickListener(View.OnClickListener {
                val productCardDetailTransitionName = "endAddProduct"
                val action =
                    Product_List_FragmentDirections.actionProductListFragmentToProductDetailFragment()
                val extras =
                    FragmentNavigatorExtras(binding.addNote to productCardDetailTransitionName)
                ...
                when (it) {
                    is NetworkResult.Success -> {
                        ...
                        if (binding.edtTextForSearch.text.isEmpty()) {
                            adapter = ProductAdapter(requireContext(), it.data!!)
                            ...
                            var filteredList = dataList.filter {
                                it.product_name.contains(
                                    binding.edtTextForSearch.text.toString(),
                                    ignoreCase = true
                                )
                            }
                            dataList.clear()
